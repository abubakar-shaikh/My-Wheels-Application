...
const Supervised = ({ navigation }) => {
  ...
  return(
    ...
    <View style={styles.header}>
      ...
      <Text style={{fontWeight:'700'}} font="h2">Supervised Cars</Text>
      </View>
      <TouchableOpacity onPress={() => navigation.navigate('Category', { title: 'Supervised Cars' ,isfor:'sCar',type:'supervised' })}>
      ...
      >
        <View style={styles.card}>
          ...
          <ImageBackground
            ...
            style={styles.image}
            />
          <Text style={{backgroundColor:'red',color:'#fff',textAlign:'center',padding:5,fontSize:11}}>PKR {Math.floor(item.price).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") }</Text>
         </View>
          <View style={{borderTopWidth:1.5,width:40,alignSelf:'center',borderColor:'red'}}>
          </View>
          <Text style={{textAlign:'center',justifyContent:'center',marginVertical:7,fontSize:11.5,fontWeight:'bold'}} color="gray100" >
            {item.brand_name} {item.model_name}
          </Text>
          <Text style={{textAlign:'center',justifyContent:'center',marginBottom:10}} color="primaryAlt" font='h5'>
            {item.state_name}
          </Text>
        </View>
      </TouchableOpacity>
